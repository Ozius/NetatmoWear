...
package netatmowear.mugitek.com.netatmoapi;
            ...
            for (int j = 0; j < modules.length(); j++) {
                ...
                for (int i = 0; i < types.length; i++) {
                    if (types[i].equals(Params.TYPE_TEMPERATURE)) {
                        ...
                        measures.setMaxTemp(getJSONString(moduleData, types[i]));
                    } else if (types[i].equals(Params.TYPE_DATE_MIN_TEMP)) {
                        String dateMinTemp = moduleData.getString(Params.TYPE_DATE_MIN_TEMP);
                        measures.setDateMinTemp((dateMinTemp == null) ? 0 : Long.parseLong(dateMinTemp) * 1000);
                    } else if (types[i].equals(Params.TYPE_DATE_MAX_TEMP)) {
                        String dateMaxTemp = moduleData.getString(Params.TYPE_DATE_MAX_TEMP);
                        measures.setDateMaxTemp((dateMaxTemp == null) ? 0 : Long.parseLong(dateMaxTemp) * 1000);
                    }
                }
